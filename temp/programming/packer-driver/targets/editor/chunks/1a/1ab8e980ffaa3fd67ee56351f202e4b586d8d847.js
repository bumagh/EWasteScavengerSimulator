      ...
      _export("Algorithm", Algorithm = class Algorithm {
        ...
        static GetRandomElements(array, count) {
          ...
          while (result.length < count) {
            const randomIndex = this.GetRandomNumber(array.length - 1, 0); // 检查该索引是否已经被使用过  
        ...
        static GetRandomItemByWeight(map, seed) {
          ...
          for (const item of map) totalWeight += item[1];
        ...
        static ConvertMapToArray(map) {
          ...
          for (const item of map) array.push(item);
        ...
        static TruncateString(target, maxLength = 4, suffix = "...") {
